    ...
    override func didReceive(_ request: UNNotificationRequest, withContentHandler contentHandler: @escaping (UNNotificationContent) -> Void) {
        ...
        if let bestAttemptContent = bestAttemptContent {
            ...
            if let imageURL = request.content.userInfo["imageURL"] as? String {
                urlString = baseAbsoluteURLString + imageURL
            }
            if urlString != nil, let fileUrl = URL(string: urlString!) {
                ...
                guard let attachment = UNNotificationAttachment.saveImageToDisk(fileIdentifier: "reference.jpg", data: imageData, options: nil) else {
